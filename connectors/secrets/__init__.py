...
Design: Never store plaintext credentials in the database.
        Store a "secret reference" like "vault://secret/db/prod" or "env://POSTGRES_PASSWORD"
        The SecretManager resolves the reference at runtime.
"""
import os
import logging
...
class SecretManager:
    ...
    def resolve(secret_ref: str) -> Optional[str]:
        ...
        Reference formats:
            ...
            plain://actual_value        → Plaintext (dev only, discouraged)
        ...
        if "://" not in secret_ref:
            # No scheme — treat as environment variable name
    ...
    def resolve_config(config: dict) -> dict:
        ...
        for key, value in config.items():
            if key.endswith("_ref") and isinstance(value, str):
                real_key = key[:-4]  # Remove '_ref' suffix
